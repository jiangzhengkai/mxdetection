...
class FPNIter(BaseIter):
    ...
    def infer_max_label_shape(self, max_data_shape):
        ...
        for i in range(len(self.config.network.rpn_feat_stride)):
            feat_shape = self.rpn_feat_sym[i].infer_shape(**dict(max_data_shape))[1][0]
    ...
    def get_label(self, **kwargs):
        ...
        feat_shape_list = []
        for i in range(len(self.config.network.rpn_feat_stride)):
            feat_shape = self.rpn_feat_sym[i].infer_shape(data=data_tensor.shape)[1][0]
            feat_shape_list.append(feat_shape)
        for i in range(self.batch_size):
            batch_labels = dict()
            label = assign_anchor_fpn(feat_shape=feat_shape_list,
                                      gt_boxes=gt_roidb_list[i]['gt_boxes'],
...
class FPNTestIter(BaseTestIter):
    ...
    def get_one_roidb(self, roidb_j, j=0):
        ...
        if self.config.TEST.use_gt_rois:
            ...
            res_dict = image_aug_function(im, **self.kwargs)
            im = res_dict['img']
            im_scale = res_dict['img_scale']
