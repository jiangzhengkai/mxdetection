...
from mxdetection.core.bbox.bbox_transform import bbox_overlaps
from mxdetection.core.bbox.nms.nms import py_nms_wrapper, gpu_nms_wrapper
from mxdetection.ops.cython.gpu_mv import mv as mask_voting_kernel
from mxdetection.ops.pycocotools import mask as mask_util
...
def polys_or_rles_to_masks(polys_or_rles, boxes, mask_height, mask_width):
    ...
    for i, ann in enumerate(polys_or_rles):
        if type(ann) == list:
            ...
            # RLE format
            mask = np.array(mask_util.decode(ann), dtype=np.float32)
