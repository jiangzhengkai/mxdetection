...
def deformable_conv(data, name, num_filter, num_deformable_group,
    ...
    offset = mx.sym.Convolution(data=data, name=name + '_offset',
                                ...
                                num_group=1, no_bias=False, attr={'__init__': init_zero.dumps()}, weight=offset_weight,
                                bias=offset_bias)
...
def residual_backbone(data,
    ...
    logging.info('stage0: {}'.format(body.infer_shape(**input_dict)[1]))
    for i in range(num_stage):
        # if units[i] - deformable_units[i] < 1:
        #     print 'stage_%d_1,deformbale conv'%(i+1),'deformbale group',deformable_group[i]
        # else:
        #     print 'stage_%d_1,normal conv'%(i+1),deformable_group[i]
        ...
        for j in range(units[i] - 1):
            # if units[i] - deformable_units[i] < j + 2:
            #     print 'stage_%d_%d, deformable conv'%(i+1,j+2),'deformbale group',deformable_group[i]
            # else:
            #     print 'stage_%d_%d, normal conv' % (i + 1, j + 2),deformable_group[i]
            body = residual_unit(data=body,
